...
const Question = () => {
  ...
  const updateAnswer = (qId, ans) => {
    console.log(context);

    const temp = [...context.selectOption];
    ...
    if (temp.length <= 5) {
      if (q !== 4) {
        setContext({ selectOption: [...temp] });
        setQ((prev) => prev + 1);
      } else {
        console.log("nothing" + timeTaken);
        setContext({ selectOption: [...temp], timeTaken });
        navigate('/result')
      }
    }
  };
      ...
      <section className="text-gray-600 body-font flex flex-col w-3/4 pt-5">
        <div className="container mx-auto flex px-5 py-10 md:flex-col">
          ...
          {qus[q]?.imgName != null && (
            <div className="lg:flex-grow md:w-full pt-5 lg:pr-24 md:pr-16 flex flex-col md:items-start md:text-left mb-16 md:mb-0 items-center text-center">
              <img
                src={BASE_URL + "images/" + qus[q]?.imgName}
            ...
            <ul>
              {qus[q]?.options?.map((option, index) => (
                <li key={index} className="py-2">
                  <a
                    href="#_"
                    onClick={() => updateAnswer(qus[q]?.qId, index)}
